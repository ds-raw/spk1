...
loaded_model = joblib.load("aws.joblib")
...
@callback(
    ...
    [State('lt-input', 'value'),
     ...
     State('Distance-Category-Label-dropdown', 'value'),
...
def update_prediction(n_clicks, lt_input, air_label_input, listrik_label_input, distance_category_input, lebar_jalan_input, clusters, peruntukan, hap, kota_kabupaten, bentuk_tapak, kondisi_wilayah, kondisi_tapak):
    if n_clicks == 0 or None in [lt_input, air_label_input, listrik_label_input, distance_category_input, lebar_jalan_input, clusters, peruntukan, hap, kota_kabupaten, bentuk_tapak, kondisi_wilayah, kondisi_tapak]:
        return None

    # Convert air_label boolean to integer (0 for False, 1 for True)
   

    # Convert distance_category to integer (3 to 0)
...
def make_predictions(model, new_data):
    ...
    all_features += ['Kondisi Tapak_Darat / Kering', 'Kondisi Tapak_Tanah Mentah', 'Kondisi Tapak_Tanah Rawa',
                     'Kondisi Tapak_Tanah Siap Dikembangkan (Tanah Matang)',
                     'Kondisi Tapak_Tanah dalam Pengembangan']
